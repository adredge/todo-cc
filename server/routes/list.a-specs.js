...
describe('/api', () => {
  ...
  before((done) => {
    userId = uuid.v4()
    agent.post('/api/auth').send({ userId: userId }).end(done)
  })
  ...
  describe('/list ', function () {
    ...
    context('When creating a new list and adding items to it', () => {
      ...
      beforeEach((done) => {
        agent.post('/api/createList').end((err, res) => {
          agent.get('/api/list').end((err, res) => {
      ...
      afterEach(() => {
        return agent.delete(`/api/deleteList/${createdList._id}`).expect(200)
      })
      ...
      it('should accept added items', () => {
        agent.post('/api/addItem').send({ listId: createdList._id, newItemName: item1Name }).end((err, res) => {
          agent.post('/api/addItem').send({ listId: createdList._id, newItemName: item2Name }).end((err, res) => {
            agent.get('/api/list').end((err, res) => {
              expect(res.items[0].name).to.equal(item1Name)
              expect(res.items[1].name).to.equal(item2Name)
            })
          })
        })
      })
